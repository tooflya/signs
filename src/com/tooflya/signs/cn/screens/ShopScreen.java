...
import com.tooflya.signs.cn.entities.List;
import com.tooflya.signs.cn.entities.Rectangle;
import com.tooflya.signs.cn.entities.ShopButton;
import com.tooflya.signs.cn.managers.ArrayEntityManager;

/**
 * @author Tooflya.com
 * @since
 */
public class ShopScreen extends Screen implements IOnSceneTouchListener {
	...
	public final Entity mScroolBar;
    ...
    public boolean onSceneTouchEvent(Scene pScene, TouchEvent pSceneTouchEvent) {
		...
		case TouchEvent.ACTION_DOWN:
			this.mList.mPostUpdate = false;

			this.mList.mLastTouchY = pTouchAreaLocalY;
			this.mList.mLastPositionY = this.mList.getY();
			break;
		case TouchEvent.ACTION_UP:
			this.mList.mPostUpdate = true;
			break;
		case TouchEvent.ACTION_MOVE:
			this.mList.setPosition(this.mList.getX(), pTouchAreaLocalY - this.mList.mLastTouchY + this.mList.mLastPositionY);
			break;
		}
	...
	public ShopScreen() {
		...
		this.mList = new List(MLIST_HEIGHT, this) ;
		...
		this.mGetButtons = new ArrayEntityManager<ShopButton>(28, new ShopButton(Resources.mBuyButtonTextureRegion, this.mList));
		...
		for (int i = 0; i < 28; i++) {
			...
			if (!isAvailable) {
				...
				this.mGetButtonBackgrounds.create().setVisible(false);
				this.mGetButtons.create().create(price, i + 1).setVisible(false);
			}
			else {
		...
		if(Options.IS_CHINA_VERSION) {
			this.mGetCoinsBackground.setVisible(false);
			this.mGetCoinsButton.setVisible(false);
		}
	}
